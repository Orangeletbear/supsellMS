...
public class CaiGouTongJiCheckDialog extends JDialog implements ActionListener {
	...
	String danJuNames[]={"所有单据","采购单","退货单"};
	/**
          ...
          Vector data = new Vector();
    		...
    		try {
    			...
    			String cangKu=cangKuBox.getSelectedItem().toString();
    	      	...
    	      	if("所有仓库".equals(cangKu)&&"所有业务员".equals(jingBanRen)&&"所有单据".equals(danJu)){
    	      		sql="(select cg_date,cg_jbr,'采购进货',cg_Id,cg_yfje,cg_sfje,tb_ckinfo.ck_name,tb_ghsinfo.ghs_name,cg_czy,null " +
			        ...
			        "(select th_date,th_jbr,'采购退货',th_Id,th_yfje,th_sfje,tb_ckinfo.ck_name,tb_ghsinfo.ghs_name,th_czy,null " +
    	     	...
    	     	else if("所有仓库".equals(cangKu)&&"所有业务员".equals(jingBanRen)&&"采购单".equals(danJu)){
    	      		sql="select cg_date,cg_jbr,'采购进货',cg_Id,cg_yfje,cg_sfje,tb_ckinfo.ck_name,tb_ghsinfo.ghs_name,cg_czy,null " +
				      "from tb_cg_main,tb_ghsinfo,tb_ckinfo where cg_ghs=tb_ghsinfo.ghs_id and cg_lkid=tb_ckinfo.ck_id " +
				      "and to_date('"+date1+"','YYYY-MM-DD')-tb_cg_main.cg_date<0 and to_date('"+date2+"','YYYY-MM-DD')-" +
    	      	...
    	      	else if("所有仓库".equals(cangKu)&&!"所有业务员".equals(jingBanRen)&&"退货单".equals(danJu)){
    	      		sql = "select th_date,th_jbr,'采购退货',th_Id,th_yfje,th_sfje,tb_ckinfo.ck_name,tb_ghsinfo.ghs_name,th_czy,null " +
                    "from tb_th_main,tb_ghsinfo,tb_ckinfo where th_ghs=tb_ghsinfo.ghs_id and th_ckId=tb_ckinfo.ck_id " +
                    "and to_date('"+date1+"','YYYY-MM-DD')-tb_th_main.th_date<0 and to_date('"+date2+"','YYYY-MM-DD')-" +
						"tb_th_main.th_date>0 and ghs_name='"+gongHuoShang+"'";
    	      	}
    	      	else if(!"所有仓库".equals(cangKu)&&"所有业务员".equals(jingBanRen)&&"所有单据".equals(danJu)){
    	      		sql="(select cg_date,cg_jbr,'采购进货',cg_Id,cg_yfje,cg_sfje,tb_ckinfo.ck_name,tb_ghsinfo.ghs_name,cg_czy,null " +
				      "from tb_cg_main,tb_ghsinfo,tb_ckinfo where cg_ghs=tb_ghsinfo.ghs_id and cg_lkid=tb_ckinfo.ck_id " +
				      "and to_date('"+date1+"','YYYY-MM-DD')-tb_cg_main.cg_date<0 and to_date('"+date2+"','YYYY-MM-DD')-" +
	...
	public void actionPerformed(ActionEvent e) {
         if(e.getSource()==chaXun){
        	 new TongJiGongHuoShangDialog(CaiGouTongJiCheckDialog.this,"查询供货商",true);
         }	
