...
import com.cn.control.richangframe.Guote_Manage.Action_sp_info_change;

public class Quote_sp_info_change extends JDialog{
	...
	public Quote_sp_info_change(Add_Quote_Goods aqgs,String title,boolean b){
		super(aqgs,title,b);
		this.aqgs = aqgs;
		init();
	...
	private void init() {
		...
		Vector v = (Vector)aqgs.getTabelModel_table_right().getDataVector().get(Action_Select_change_del.row);
		...
		box_3_1.add(wfbj);
		//		Vector v1 = (Vector)aqgs.getData_left_tab_1().get(aqgs.getTable_tab_1().getSelectedRow());
		...
		//确定添加
		comfirm.addActionListener(new Action_sp_info_change(Quote_sp_info_change.this));
